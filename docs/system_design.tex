...
\begin{document}
	...
	booting,
	communicating,
	and attesting have different numbers.
	...
	The comparison time is constant regardless of the content of the PIN or Replacement Token to mitigate a side-channel attack.
	...
	A CP does not have the decryption process during runtime.
	...
	\begin{enumerate}
		...
		the CP makes up the response by signing a boot purpose label (a 1-byte number marks the purpose is booting),
		the challenge,
		...
		\item To boot a CP,
		the AP finds the nonce in the challenge sent by this CP before.
	...
	The technician in a repair station needs to tell the AP which CP ID are no longer provisioned and what the new CP ID with supplying the correct replacement Token.
	...
	so decrypting is necessary when retrieving the data.
	Also,
	...
	\begin{enumerate}
		...
		The comparison time is always constant no matter the PIN correctness.
	...
	\begin{enumerate}
		...
		The AP sends the response,
		message,
		and a signature of the message (signed using the \texttt{K\_Pri1} key) to th CP.
	...
	\begin{enumerate}
		...
		message,
		and a signature of the message signed using the \texttt{K\_Pri2} key in the I2C buffer,
